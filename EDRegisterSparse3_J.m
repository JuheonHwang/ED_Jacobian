        ...
        Jlap = sparse(numel(Elap), numel(P0));
            ...
            for i = 1:n_nodes
                for j = 1:9
                    ...
                    Jfit(:, 6 + j + 9 * (i - 1)) = reshape(gather(new_deformedV_local(idxNearest,:) * R0'), [], 1);
                    Jsmooth(:, 6 + j + 9 * (i - 1)) = reshape(gather(sum(squeeze(pagemtimes(new_A0, v_node1)) + g0 + t0 - g_node1 - t_node1, 3)), [], 1);
                    Jlap(:, 6 + j + 9 * (i - 1)) = reshape(gather(Lap * new_deformedV_local * R0'), [], 1);
                end
                for k = 1:3
                    ...
                    Jfit(:, 6 + nA + k + 3 * (i - 1)) = reshape(gather(new_deformedV_local(idxNearest,:) * R0'), [], 1);
                    Jsmooth(:, 6 + nA + k + 3 * (i - 1)) = reshape(sum(new_t0 - new_t_node1, 3), [], 1);
                    Jlap(:, 6 + nA + k + 3 * (i - 1)) = reshape(gather(Lap * new_deformedV_local * R0'), [], 1);
                end
                A = A0(:, :, i);
                A = A';
