...
function [F, J] = EfuncLocal(P0, SrcMesh, pntTar, G0, nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, Lap, w_rot, w_smooth, w_lap, R_, T_, A_, t_, indices)  
    ...
    if nargout > 1
        ...
        Jfit = zeros(numel(Efit), numel(P0));
        ...
        smooth_A_row = reshape(repmat(1:3*n_nodes, [3, 1]), [], 1);
        smooth_A_col = reshape((1:nA), [], 1);
        smooth_A_val = gather(reshape(repmat(reshape(squeeze(sum(v_node1, 4)), 1, []), [3, 1]), [], 1));
        ...
        smooth_t_val = [];
        for i = 1:n_nodes
            [row, col] = find(idxNodeNeighbor == i);
            smooth_t_row = [smooth_t_row; 3*(col-1)+1; 3*(col-1)+2; 3*(col-1)+3];
            smooth_t_col = [smooth_t_col; reshape(repmat((3*(i-1)+1):(3*i), [length(col), 1]), [], 1)];
            vals = repmat(-1, [length(col), 1]);
            vals(row == 1) = 5;
            smooth_t_val = [smooth_t_val; reshape(repmat(vals, [1, 3]), [], 1)];
        end
