...
[src_face, src_pts] = jwutils.plyread('simple(f80000)_poisson_mesh_020.ply', 'tri');
[tar_face, tar_pts] = jwutils.plyread('simple(f80000)_poisson_mesh_022.ply', 'tri');
rng('default')
s = rng;
...
function [F, J] = EfuncLocal(P0, SrcMesh, pntTar, G0, nodeIdx, idxNeighbor, idxNodeNeighbor, distNeighbor, Lap, w_rot, w_smooth, w_lap, R_, T_, A_, t_, indices)  
    ...
    if nargout > 1
        ...
        Px = (rotationVectorToMatrix(P0(1:3)'+[1, 0, 0] * jacobian_delta)-rotationVectorToMatrix(P0(1:3)'));
        Py = (rotationVectorToMatrix(P0(1:3)'+[0, 1, 0] * jacobian_delta)-rotationVectorToMatrix(P0(1:3)'));
        Pz = (rotationVectorToMatrix(P0(1:3)'+[0, 0, 1] * jacobian_delta)-rotationVectorToMatrix(P0(1:3)'));
        ...
        J_ = [Jfit; Jrot1; Jrot2; Jsmooth; Jlap];
            ...
            Jfit(:, 1) = reshape(gather((deformedV_local(indices, :) - G0(:)') * Px'), [], 1);
            Jfit(:, 2) = reshape(gather((deformedV_local(indices, :) - G0(:)') * Py'), [], 1);
            Jfit(:, 3) = reshape(gather((deformedV_local(indices, :) - G0(:)') * Pz'), [], 1);
            Jfit(1:n_target, 4) = 1;
        ...
        Jrot1_ = sparse(w_rot *  Jrot1(:, 7:end) ./ sqrt(n_nodes));
        Jrot2_ = sparse(w_rot *  Jrot2(:, 7:end) ./ sqrt(n_nodes));
        ...
        J_(1:(3 * n_target), 1:6) = sparse(Jfit(:, 1:6) ./ sqrt(n_target));
        J_((1 + 3 * n_nodes + n_nodes + 3 * 3 * n_nodes + 3 * n_target):end, 1:6) = sparse(w_lap * Jlap(:, 1:6) ./ reshape(repmat(full(diag(gather(Lap))), [1 3]), [], 1));
