...
TarMesh.Vpoi = src_pts + 3 * randn(size(src_pts)); TarMesh.Fpoi = src_face;
rng(1, 'twister')
...
%             Jrot2 = sparse(rot2_rows, rot2_cols, rot2_vals, numel(Erot2), numel(P0));
        ...
        Jlap_ = sparse(w_lap * J_((1 + 3 * n_nodes + n_nodes + 3 * 3 * n_nodes + 3 * n_target):end, 7:end) ./ reshape(repmat(full(diag(gather(Lap))), [1 3]), [], 1));
