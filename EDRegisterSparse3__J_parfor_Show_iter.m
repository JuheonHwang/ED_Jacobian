        ...
        Jlap = sparse(numel(Elap), numel(P0));
            ...
            parfor i = 1:(12 * n_nodes)
                if (i <= nA)
                    ...
                    Jsmooth(:, 6 + i) = reshape(gather(sum(squeeze(pagemtimes(new_A0, v_node1)), 3)), [], 1);
...
%             Jrot1 ./ sqrt(n_nodes);
%             Jrot2 ./ sqrt(n_nodes);
%             Jsmooth ./ sqrt(3 * n_nodes);
%             Jlap ./ reshape(repmat(full(diag(gather(Lap))), [1 3]), [], 1)];
